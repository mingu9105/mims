...
class BeforeLV(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        return Medimage.objects.filter(requesterID=self.request.user, progress=0)
...
class AfterLV(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        return Medimage.objects.filter(requesterID=self.request.user, progress=1)
...
class PostCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):  # self.request
        ...
        form.instance.requesterID = self.request.user
